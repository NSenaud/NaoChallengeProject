...
void automate ()
{
  ...
  const unsigned char servoFinal = 90;
  ...
  while (poidsActuel <= seuilMax)  // continuer tant que l'on n'a pas atteint le premier seuil
  {
    GLCD.SetDot(24, 8, BLACK);
    poidsActuel = abs(scale.get_units());
    poidsTronque = int (poidsActuel * 10);
    poidsTronque /= 10.0;
...
void affichageGLCD (float poids, unsigned char nbDecimale)
{
  ...
  pourcentPoids = int((poids / (10.f)) * 100);
  GLCD.ClearScreen();
  GLCD.CursorTo(5, 1);
